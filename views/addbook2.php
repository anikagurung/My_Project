...
if (isset($_POST['submit'])) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
            // Insert product details into the pending products table
            if (isset($sellerId)) {
                $sql = "INSERT INTO pending_products (seller_id, title, authorname, price, photo) VALUES (?, ?, ?, ?, ?)";
                $stmt = $mysqli->prepare($sql);
                if ($stmt) {
                    $stmt->bind_param("issss", $sellerId, $title, $authorname, $price, $target_file);
                    if ($stmt->execute()) {
                        // Product added successfully
                        echo '<script>alert("Product added successfully. Your product will be reviewed by an admin before it is displayed on the website.");';
